...
export function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--dry-run" || arg === "--prerelease") {
      args[arg.slice(2).replace(/-([a-z])/g, (_, letter) => letter.toUpperCase())] = true;
      continue;
    }
  ...
  for (const requiredArg of ["repository", "tag", "target", "previousTag", "name", "imageName", "dockerTagsFile"]) {
    if (!args[requiredArg]) {
      throw new Error(`Missing --${requiredArg.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)}.`);
    }
  }
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const generatedNotes = await githubApi(`/repos/${args.repository}/releases/generate-notes`, {
    method: "POST",
    token,
    body: {
      tag_name: args.tag,
