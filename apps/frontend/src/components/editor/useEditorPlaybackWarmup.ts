...
export function useEditorPlaybackWarmup({
  ...
  const warmClipStart = useCallback(
    async (clipStart: number) => {
      if (
        loadingPreview ||
        playingRef.current ||
      ...
      const sourceTimestamp = toSourceTimelineTime(
        displayTimestamp,
      ...
      const warmupPromise = (async () => {
        ...
        try {
          await Promise.allSettled([
            videoSink
              ? videoSink.getCanvas(sourceTimestamp, packetRetrievalOptions)
              : Promise.resolve(null),
            audioSink
              ? audioSink.getBuffer(sourceTimestamp, packetRetrievalOptions)
              : Promise.resolve(null),
          ]);
        } catch {
