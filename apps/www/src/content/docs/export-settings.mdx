...
Provider-backed clips can include title, show, season, episode, artwork, rating, and Cliparr timing metadata when the source provides it. Local files and direct URLs include only best-effort metadata. See [Export metadata](/docs/export-metadata) for the full metadata table.
