...
Use the workflow's dry-run mode before publishing. Real releases require the `CLOUDFLARE_PAGES_DEPLOY_HOOK_URL` repository secret. Cloudflare Pages production builds should also have a read-only `GITHUB_TOKEN` or `GH_TOKEN` environment variable so the changelog mirror does not hit unauthenticated GitHub API rate limits.
